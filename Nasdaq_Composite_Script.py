# file: nasdaq_multi_method_screener.py
import argparse
import warnings
...
def screen_nasdaq_multi_methodology(
    ...
    """
    Screen NASDAQ stocks for buy signals from ALL methodologies.
    Uses fast chunked downloads and a global PRICE_DATA cache.
    """
    tickers = _tickers_override or get_nasdaq_composite_tickers()
...
def _print_console(results, total_screened):
    print("\n" + "=" * 80)
    print("🎯 NASDAQ STOCKS WITH BUY SIGNALS FROM ALL METHODOLOGIES")
    print("=" * 80)
...
def run_streamlit():
    ...
    st.set_page_config(page_title="NASDAQ Composite Multi-Method Screener", layout="wide")
    st.title("📈 NASDAQ Composite Multi-Methodology Screener")
    ...
    st.download_button(
        ...
        file_name="nasdaq_multi_methodology_buy_signals.csv",
        mime="text/csv",
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="NASDAQ multi-method screener (CLI or Streamlit).")
